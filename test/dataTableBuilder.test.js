...
function makeXYZTestData(numEntries) {
  return Array.from({ length: numEntries }, (_, idx) => ({
    x: `x${idx}`,
    y: `y${idx}`,
    z: `z${idx}`,
  }));
...
test("buildDataTableDataOnly", () => {
  ...
  let dataRows = Array.from(
    dataset,
    (entry) =>
      `<tr><td>${entry.x}</td><td>${entry.y}</td><td>${entry.z}</td></tr>`,
  );
...
test("buildDataTableNoRowNames", () => {
  ...
  let expected = `<table><thead>${headerRow}</thead><tbody>${dataRows.join(
    "",
  )}</tbody></table>`;
  expect(result).toEqual(expected);
});
...
test("buildDataTableNoColNames", () => {
  ...
  let dataRows = Array.from(
    dataset,
    (entry) =>
      `<tr><th scope="row">${rowNames.shift()}</th><td>${entry.x}</td><td>${
        entry.y
      }</td><td>${entry.z}</td></tr>`,
  );
  let expected = `<table><tbody>${dataRows.join("")}</tbody></table>`;
...
test("buildDataTableScalarData", () => {
  ...
  let dataRows = Array.from(
    dataset,
    (entry) =>
      `<tr><th scope="row">${rowNames.shift()}</th><td>${entry}</td></tr>`,
  );
  let expected = `<table><thead>${headerRow}</thead><tbody>${dataRows.join(
    "",
  )}</tbody></table>`;
...
test("buildDataTableInsufficientColNames", () => {
  ...
  let dataRows = Array.from(
    dataset,
    (entry) =>
      `<tr><th scope="row">${rowNames.shift()}</th><td>${entry.x}</td></tr>`,
  );
  let expected = `<table><thead>${headerRow}</thead><tbody>${dataRows.join(
    "",
  )}</tbody></table>`;
...
test("buildDataTableInsufficientRowLabels", () => {
  ...
  let result = buildDataTable(rowNames, colNames, dataset);
  let headerRow = `<tr><th scope="col">x</th><th scope="col">y</th><th scope="col">z</th></tr>`;
  let rowHeaders = rowNames
    .map((name) => `<th scope="row">${name}</th>`)
    .concat(Array.from({ length: 5 }, () => ""));
  let dataRows = Array.from(
    dataset,
    (entry) =>
      `<tr>${rowHeaders.shift()}<td>${entry.x}</td><td>${entry.y}</td><td>${
        entry.z
      }</td></tr>`,
  );
  let expected = `<table><thead>${headerRow}</thead><tbody>${dataRows.join(
